...
async function fetchClientSearchResults(query: string): Promise<BdcClientTableRow[]> {
  if (!query || query.length < 3) return [];
  
  const response = await fetch(`/api/public/clients/search?query=${encodeURIComponent(query)}`);
  if (!response.ok) {
...
export function ClientSearchResultsList() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col gap-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex flex-col md:flex-row border rounded-xl overflow-hidden h-[200px] md:h-[160px]">
  ...
  return (
    <div className={`mt-4 flex flex-col gap-6 transition-opacity duration-200 ${isFetching ? 'opacity-70' : 'opacity-100'}`}>
      {results.map((row) => (
